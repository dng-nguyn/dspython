...
| SARIMAX (log)        | 402.014   | 512.715   | 26,87% | −0,71    |
...
**Ghi chú phương pháp dự báo.** Các mô hình dựa trên cây (Hồi quy tuyến tính, Random Forest, XGBoost) dựa vào `lag_1` làm đặc trưng chính. Trên tập kiểm tra 24 tháng (2024--2025), giá trị `lag_1` thực tế của mỗi tháng đều có sẵn, nên các mô hình này thực chất thực hiện dự đoán một bước. Để tạo dự báo trước 12 tháng thực sự, các mô hình này sẽ cần một chiến lược đệ quy: dự báo tháng $t$, sau đó đưa dự báo đó trở lại làm đặc trưng `lag_1` cho tháng $t+1$. Cách tiếp cận này tích lũy sai số tại mỗi bước và không được sử dụng ở đây. Thay vào đó, dự báo 12 tháng cho năm 2026 được tạo độc quyền bởi SARIMAX, có cấu trúc tự hồi quy và mùa vụ tự nhiên tạo ra dự báo đa bước mà không cần đầu vào lag bên ngoài. Tỷ giá hối đoái và các đặc trưng bên ngoài khác đã bị loại khỏi dự báo ngoài mẫu để tránh rò rỉ dữ liệu.
...
![Dự báo 12 tháng của SARIMAX cho 5 quốc gia nguồn hàng đầu (2026, biến đổi log).](output/country_forecasts_plot.png)
...
Với dữ liệu thực tế có sẵn cho 5 tháng đầu năm 2026, chúng ta có thể đánh giá dự báo SARIMAX so với thực tế.
