...
- **Random Forest đạt MAPE tốt nhất** (8,87\\%, MAE = 143.971), tiếp theo là Linear Regression (9,68\\%, MAE = 154.368). Đây là hai mô hình duy nhất có R$^2$ dương (0,30 và 0,36), cho thấy chúng trích xuất được tín hiệu thực từ dữ liệu.
- **Chronos-T5-small** đạt MAPE cạnh tranh 10,77\\% chỉ sử dụng giá trị chuỗi thời gian thô (không có đặc trưng kỹ thuật), nhưng R$^2$ âm ($-0,03$) phản ánh khó khăn trong việc ngoại suy xu hướng tăng trưởng sau COVID-19 từ một mô hình nền tảng được huấn luyện trước.
- **Random Forest:** `lag_1` chiếm 69,8\\% mức độ quan trọng của đặc trưng, tiếp theo là `lag_12` (11,3\\%) và `time_idx` (8,7\\%). Sự thống trị của `lag_1` có nghĩa là các mô hình dựa trên cây hoạt động chủ yếu như các mô hình dự báo naive một bước.

- **Lưu ý quan trọng: dự báo một bước vs đa bước.** Bảng so sánh mô hình trên đánh giá độ chính xác *một bước trước*: mỗi tháng kiểm tra sử dụng giá trị lag thực tế của tháng trước. Dự báo 2026 yêu cầu dự báo *đa bước đệ quy* với sai số tích lũy. Các chỉ số một bước đại diện cho giới hạn trên của hiệu suất dự báo.
- **SARIMAX** (MAPE = 20,39\\%, R$^2$ = $-1,85$) với mục tiêu biến đổi log $z_t = \\log(y_t+1)$ và biến exogenous `covid_closed`. Biến đổi log ổn định phương sai và đảm bảo biến đổi ngược không âm, nhưng mô hình vẫn gặp khó khăn trong việc ngoại suy xu hướng sau COVID-19.
- **CIR#** (MAPE = 25,12\\%, R$^2$ = $-3,94$) được đưa vào để xác nhận thực nghiệm điều kiện biên [13, 14]: mô hình giả định quá trình dừng, hồi quy về trung bình. $\\kappa$ ước lượng dương nhưng dữ liệu có xu hướng mạnh vi phạm giả định này, tạo ra dự báo gần bằng không.
